...
do
  ...
  for i in $(dig $domain_name NS |grep NS|grep -v ';'| sed 's/[[:blank:]]/;/g'|cut -d ';' -f 6)
    do
      ...
      # get the IP address assigned to the DNS (grep -E filters on IPv4 address only)
      ip=$(nslookup $ns |grep Address|grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}"|tail -n1|cut -d ':' -f 2|sed 's/ //g')
